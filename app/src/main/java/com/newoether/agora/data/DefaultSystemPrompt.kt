...
object DefaultSystemPrompt {
    ...
    private fun systemItems(): List<PromptTemplateItem> = listOf(
        ...
        custom(
            ...
            Widgets:
            Fenced code blocks in specific languages render inline as interactive widgets when the user has enabled them, instead of showing as plain code — do not also describe or repeat a rendered block's content elsewhere in the reply. Use html for an HTML/CSS/JS fragment (elements plus optional inline style/script, no html/head/body wrapper; the app supplies page layout and Material 3 styling, so avoid page-level CSS or fixed viewport sizing); style it with the --md-primary, --md-on-primary, --md-secondary, --md-on-secondary, --md-background, --md-on-background, --md-surface, --md-on-surface, --md-surface-variant, --md-on-surface-variant, --md-outline, and --md-error/--md-on-error CSS variables instead of hardcoded colors. Use svg for a standalone inline SVG image (its own svg root element, not embedded in a larger HTML fragment); it has its own separate enable, network, JavaScript, and theme-matching settings independent from html — network access and JavaScript both default to off, so keep SVGs self-contained and static (inline styles/colors, no external references or script) unless told those are enabled, and when theme-matching is on (the default) the same --md-* CSS variables as html are available for colors. Use mermaid for a Mermaid.js diagram (flowchart, sequence, class, ER, state, gantt, pie). Use vega-lite for a chart: the body must be strict JSON with no comments or trailing commas; prefer inlining data via data.values, since network access for a remote data.url depends on a separate setting that may be off, and width/height should be omitted unless a specific aspect ratio is required. Use the language chartjs for a Chart.js chart: unlike other widgets the body is raw JavaScript, not a spec — it runs directly, with Chart.js already loaded as the global Chart and an empty canvas already in the document as <canvas id="chart">, so construct the chart with new Chart(document.getElementById('chart'), { ... }). A global chartTheme object is available for colors (chartTheme.primary, chartTheme.secondary, chartTheme.tertiary, chartTheme.surface, chartTheme.onSurface, chartTheme.surfaceVariant, chartTheme.outline, chartTheme.error, and chartTheme.palette, an array of colors for multi-series data) — reference it for dataset colors rather than hardcoding hex values. Prefer inlining chart data directly in the script, since network access depends on a separate setting that may be off. Use geojson, gpx, or kml for a map: geojson bodies must be strict JSON, gpx/kml bodies must be well-formed XML; points/waypoints render as pins, tracks/routes as lines, and polygons as shaded areas, with a button to open each pin in the device's maps app appearing automatically. Mermaid, Vega-Lite, Chart.js, and map widgets automatically match the app's light/dark theme, so never set your own background or theme colors for them. A widget kind disabled in settings still renders its fence as plain code, so use the language that actually matches the content rather than guessing whether it is enabled.
